...
exports.create = async(req,res)=>{
    try {
        ...
        if (data.length >0) {
            const result = await bookApointment.update(JSON.stringify(cart_item),cart_name,total,lab_id,created_by_id);
        }else{
            const result = await bookApointment.create(lab_id,created_by_id,JSON.stringify(cart_item),cart_name,total);
        }
        return res.status(200).json({
            status_code:200,
            status:"success",
        ...
        for (const item of data) {
            ...
            for (const [key, value] of Object.entries(cart_item)) {
                if (Object.keys(value)[0]=="package_id") {
                    ...
                    let image_url = pData.image!=null ? process.env.APP_URL+"member/"+pData.image:"";
            ...
            const lab_image_url =  labdata!=undefined && labdata.profile_image!=undefined ?process.env.APP_URL+"member/"+labdata.profile_image:null;
...
exports.appointmentList = async(req,res)=>{
    try {
        ...
        for (const item of result) {
            ...
            data.push({id,user_id,member_id,created_by_id,first_name,pin_code,promo_code_id,appointment_date,time_slot,
...
exports.viewDoctorAvailability = async (req, res) => {
    ...
    let bookedSlots=[];
    let St = await  helperQuery.All(`SELECT * FROM appointments WHERE doctor_id ='${doctor_id}' AND status != 'Cancelled' AND DATE_FORMAT(appointment_date,"%y-%m-%d")= DATE_FORMAT('${date}',"%y-%m-%d")`);
    if (St.length>0) {
        St.map((item)=>{
