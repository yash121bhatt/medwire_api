...
exports.create = async (req, res) => {
    const { test_category_id, lab_id, test_name, test_report, fast_time, test_recommended, description, amount } = req.body;
    ...
    const image = req.file != undefined ? await uploadFileIntoCloudinary(req) : null;

    LabTest.create(test_category_id, lab_id, user_id, member_id, test_name, test_report, fast_time, test_recommended, image, description, amount, (err) => {
        if (err) {
...
exports.update = async (req, res) => {
    const { role_id, test_id, lab_id, test_category_id, test_name, test_report, fast_time, test_recommended, description, amount } = req.body;

    const image = req.file != undefined ? await uploadFileIntoCloudinary(req) : null;

    let vali;
    if (role_id == 3) {
        vali = helperFunction.customValidater(req, { role_id, test_id, lab_id, test_category_id, test_name, test_report, fast_time, test_recommended, description, amount });
        if (vali) {
            return res.status(500).json(vali);
        }
    } else if (role_id == 4) {
        vali = helperFunction.customValidater(req, { role_id, test_id, lab_id, test_category_id, test_name, description, amount });
        if (vali) {
            return res.status(500).json(vali);
        }
    }

    LabTest.update(test_category_id, test_name, test_report, fast_time, test_recommended, image, description, amount, test_id, lab_id, (err) => {
        if (err) {
            return res.status(500).json({
                status_code: "500",
                status: "error",
...
exports.show = (req, res) => {
    ...
    LabTest.show(lab_id, (err, data) => {
        ...
        data.map((item) => {
            ...
            const image = item.image != undefined && item.image != null ? process.env.CLOUDINARY_BASE_URL + "laboratory/" + item.image : "-";
