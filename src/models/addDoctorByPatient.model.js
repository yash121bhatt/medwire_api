...
class PatientsDoctor {
    ...
    static doctorList(user_id)
    {
        return new Promise((resolve,reject)=>{
            db.query(`SELECT 
            p.id,dc.clinic_id,p.created_by_id as patient_id,
            u.first_name as doctor,u.profile_image as doctor_profile,u.id as doctor_id,u.experience_in_year,
            c.first_name as clinic,c.profile_image as clinic_profile,
            pnt.first_name as patient_name, pnt.profile_image as patient_image 
            ...
            WHERE p.created_by_id ='${user_id}' and p.deleted_at IS NULL`,
            (err,res)=>{
                if(err){
                    return reject(err);
                }
