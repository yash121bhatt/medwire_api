...
class addByDoctor {
    ...
    static labRadio({ role_id, search, ids }) {
        return new Promise((resolve, reject) => {
            ...
            if (search != undefined && search != "") {
                que = "SELECT * FROM users WHERE approve_status='Approve' AND role_id=" + roleId + " AND (first_name LIKE '%${search}%') AND id NOT IN (" + cids + ") ORDER BY id DESC";
            } else {
                que = "SELECT * FROM users WHERE approve_status='Approve' AND role_id=" + roleId + " AND id NOT IN (" + cids + ") ORDER BY id DESC";
            }
    ...
    static AddlabRadio({ user_id, doctor_id }) {
        return new Promise((resolve, reject) => {
            db.query(
                "INSERT INTO radio_lab_doctors(user_id, doctor_id, created_at) VALUES(?,?,NOW())",
                [user_id, doctor_id],
                (err, res) => {
                    if (err) {
                        return reject(err);
