...
class bookApointment {
    ...
    static createOld(lab_id, user_id, cart_item, cart_name, total_amount) {
        return new Promise((resolve, reject) => {
            db.query(
                "INSERT INTO user_carts (user_id, created_by_id, cart_item, cart_name, total_amount, created_at) VALUES (?,?,?,?,?,NOW())",
        ...
        return new Promise((resolve, reject) => {
            ...
            db.query(
                ...
                (err, res) => {
                    ...
                    const insertedId = res.insertId; // get the auto-incremented ID

                    // Step 2: Update cart_id = id
                    db.query(
                        "UPDATE user_carts SET cart_id = ? WHERE id = ?",
                        ...
                        (err2, res2) => {
                            ...
                            return resolve({ insertResult: res, updateResult: res2 });
                        }
                    );
                }
            );
    ...
    static show(created_by_id) {
        return new Promise((resolve, reject) => {
            db.query(
                `SELECT * FROM user_carts WHERE created_by_id='${created_by_id}' AND status='0' AND appointment_id IS NULL`,
                (err, res) => {
                    if (err) {
                        return reject(err);
                    }
                                            ...
                                            db.query("select first_name,role_id from users where id = ? ", [refer_by_id], async (err, res) => {
                                                if (res) {
                                                    ...
                                                    if (user_role_id == 5) {
                                                        ...
                                                        db.query("INSERT INTO system_notifications(from_user_id,to_user_id,title,type,message,created_at) VALUES(?,?,?,?,?,?)", [from_user_id, to_user_id, title, type, message, created_at], async (err) => {
                                                            if (err) {
                                                                logger.error(err.message);
                                                                cb(err, null);
                                                                return;
    ...
    static appointmentList(user_id, role_id) {
        return new Promise((resolve, reject) => {
            let query;
            if (role_id == 4) {
                query = `SELECT ap.payment_order_id,ap.payment_status,ap.id,ap.user_id,ap.member_id,ap.created_by_id,ap.promo_code_id,ap.appointments_user_type,
                ...
                query = `SELECT ap.payment_order_id,ap.payment_status,ap.id,ap.user_id,ap.member_id,ap.created_by_id,ap.promo_code_id,
                p.first_name,p.pin_code ,pm.pin_code as mpin_code,
    ...
    static allDoctorSearchOld({ doctor_name, speciality, clinic_name, pin_code }) {
        ...
        if ((pin_code != undefined && pin_code != null && pin_code != "")) {
            query = `SELECT dc.id,dc.clinic_id,
    ...
    static viewDoctorAvailability({ doctor_id, clinic_id, date, bookedSlots }, cb) {
        ...
        db.query("SELECT * FROM doctor_schedule WHERE days = ? and doctor_id = ? and clinic_id = ?", [day_name, doctor_id, clinic_id], async (err, res) => {
            console.log("doctor_schedule ------------", res);
            if (res) {
