...
class LabTest {
    ...
    static findTestlist({lab_id,category_name,cat_id},cb){
        ...
        FROM lab_tests l LEFT JOIN test_categories t 
        ON l.test_category_id=t.cat_id 
        WHERE l.lab_id='${lab_id}' AND (t.category_name= '${category_name}' OR t.cat_id= '${cat_id}') order by l.test_id desc`,
            (err, res) => {
                if (err) {
                    logger.error(err.message);
                    cb(err,null);
