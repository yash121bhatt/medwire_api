    ...
    static addUserSymptom(user_id, member_id, symptom_id, status, cb) {
        helperQuery.get({ table: "symptom_user", where: "user_id=" + user_id + " AND member_id=" + member_id + " AND symptom_id =" + symptom_id }, (err, data) => {
            if (data) {
                let qu;
                if (data.length > 0) {
                    qu = `UPDATE symptom_user SET status ='${status}', updated_at=NOW() WHERE user_id='${user_id}' AND member_id='${member_id}' AND symptom_id='${symptom_id}'`;
                } else {
                    qu = `INSERT INTO symptom_user( user_id, member_id, symptom_id, status, created_at,updated_at) VALUES(?,?,?,'${status}',NOW(),NOW())`;
                }
                db.query(qu,
