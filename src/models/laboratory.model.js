...
class Laboratory {
    ...
    static newVisit(mobile, lab_id, member_id, category, sub_category, cb) {
        db.query(
            "INSERT INTO new_visit(mobile, lab_id, member_id, category, sub_category, created_at) VALUES(?,?,?,?,?,NOW())",
    ...
    static visitList(lab_id, cb) {
        ...
        WHERE lab_id = '${lab_id}' AND (online_ofline_status='0' OR online_ofline_status IS NULL )  ORDER BY visit_id DESC`,
            ...
            (err, res) => {
                ...
                if (res) {
                    // map over results and create array of promises
                    const dataPromises = res.map(async (item) => {
                        const report_document_url = await helperFunction.is_file_exist(item.report_document);
                        return {
                            category: item.category,
                            created_by_id: item.created_by_id,
                            lab_id: item.lab_id,
                            ...
                            dcm_document: item.dcm_document ?? null,
                            dcm_document_url:
                                item.dcm_document != null ? process.env.APP_URL_DCM + "/" + item.dcm_document : null,
                        };
                    });

                    // resolve all promises then call callback
