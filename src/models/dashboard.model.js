...
class dashboard {
    ...
    static dashboarChart(lab_id, type, cb) {
     ...
     `,
            ...
            db.query(`SELECT Month,monthName,total from 
    ...
    static appointmentGraph() {
        return new Promise((resolve, reject) => {
            db.query(`SELECT Date,DayName,current_total,SUM(current_total) OVER (ORDER BY CurrentDate) rtotal,
            ...
            FROM ( SELECT DATE_FORMAT(created_at,"%d-%m-%y") AS CurrentDate, COUNT(DATE_FORMAT(created_at,"%d-%m-%y")) AS current_total, created_at AS Date, DAYNAME(created_at) AS DayName 
                 ...
                 GROUP BY DATE_FORMAT(created_at,"%d-%m-%y") )r;`, (err, res) => {
                if (err) {
                    return reject(err);
                }
                return resolve(res);
    ...
    static clinicCard(clinic_id) {
        return new Promise((resolve, reject) => {
            ...
            (SELECT COUNT(*) FROM appointments WHERE clinic_id='${clinic_id}' AND DATE_FORMAT(created_at,"%y-%m-%d")=DATE_FORMAT(NOW(),"%y-%m-%d") AND payment_status='Success') as todays_apppintment`, (err, res) => {
                if (err) {
                    return reject(err);
                }
                return resolve(res);
